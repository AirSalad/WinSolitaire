...
CLevel::CLevel(): m_iBricksRemaining(0), m_pPaddle(0), m_pBall(0), m_iWidth(0), m_iHeight(0)
{
}
...
void CLevel::ProcessBallPaddleCollision()
{
	...
	if ((fBallX + fBallR > fPaddleX - fPaddleW / 2) &&(fBallX - fBallR < fPaddleX + fPaddleW / 2) &&(fBallY + fBallR > fPaddleY - fPaddleH / 2) &&(fBallY - fBallR < fPaddleY + fPaddleH / 2))
	{
...
void CLevel::ProcessBallBrickCollision()
{
	for (unsigned int i = 0; i < m_vecBricks.size(); ++i)
	{
		if (!m_vecBricks[i]->IsHit())
		{
			...
			if ((fBallX + fBallR > fBrickX - fBrickW / 2) &&(fBallX - fBallR < fBrickX + fBrickW / 2) &&(fBallY + fBallR > fBrickY - fBrickH / 2) &&(fBallY - fBallR < fBrickY + fBrickH / 2))
			{
